...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument('task', default='ctdet',
                                 help='ctdet | ddd | multi_pose | exdet | rear_headlight_multi_pose')
        self.parser.add_argument('--dataset', default='coco',
                                 help='coco | kitti | coco_hp | pascal | rear_headlight_hp')
        ...
        self.parser.add_argument('--scale', type=float, default=0.0,  # shift:0.1,scale:0.4
                                 help='when not using random crop'
    ...
    def update_dataset_info_and_set_heads(self, opt, dataset):
        ...
        if opt.task == 'exdet':
            ...
            opt.heads = {'hm': opt.num_classes,  # (C + 4)
        ...
        elif opt.task == 'rear_headlight_multi_pose':  # 1 hp point
            opt.flip_idx = dataset.flip_idx
            opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 2}
            if opt.reg_offset:
                opt.heads.update({'reg': 2})
            if opt.hm_hp:
                opt.heads.update({'hm_hp': 1})
            if opt.reg_hp_offset:
                opt.heads.update({'hp_offset': 2})
        else:
